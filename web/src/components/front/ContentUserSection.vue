<template>
    ...
    <form class="d-flex " role="search" style="width: 100%;" @submit="handleSearch">
      ...
      <button class="btn custom-search-btn" type="submit">
        <svg t="1728041224119" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg"
          p-id="5573" width="20" height="20">
          <path d="M351.1808 59.2896A435.2 435.2 0 0 1 805.376 715.264 460.8 460.8 0 0 1 351.1808 59.3408z"
            fill="#ffffff" p-id="5574" data-spm-anchor-id="a313x.search_index.0.i12.117c3a817iUd2m" class="selected">
          </path>
          <path
            ...
            fill="currentColor" p-id="5575"></path>
        </svg>
      </button>
    </form>
    <br>
  ...
  <div class="content-section-category-container">
    ...
    <span v-for="category in topLevelCategories" :key="category.id" class="capsule"
      ...
      @click="selectCategory(category);" :title="category.description">
      {{ category.name }}
    </span>
  </div>
  ...
  <div v-if="selectedCategory" class="content-section-category-container" style="margin-top: 10px;">
    <RecursiveCategory :category="selectedCategory" :now-category="nowCategory" @select-category="selectCategory" />
  </div>

  <div class="row apps-container">
    <div v-for="(app, index) in appList" :key="index" class=" col-md-3 col-lg-3 col-sm-4 col-4 app-container"
...
export default {
  ...
  setup() {
    ...
    if (token && token !== '') {
      ...
      console.log("now", nowUser.value.username);
      appListDTO.value.username = nowUser.value.username;
    }
    else {
      console.log("未登录");
      appListDTO.value.username = 'admin';
    }
...
.custom-search-btn:focus {
  box-shadow: 0 0 0 0.1rem rgba(222, 118, 34, 0.5);
  /* 添加聚焦时的阴影效果 */
}
