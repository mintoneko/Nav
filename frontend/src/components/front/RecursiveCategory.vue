<template>
  <div class="content-section-category-container" style="margin-top: 10px;">
    <!-- 当前分类的直接子分类 -->
    <span v-for="subCategory in category.children" :key="subCategory.id" class="capsule"
      @click="selectCategory(subCategory)"
      :class="{ 'capsule-active': nowCategory && nowCategory.id === subCategory.id }" :title="subCategory.description">
      {{ subCategory.name }}
    </span>
  </div>
</template>
