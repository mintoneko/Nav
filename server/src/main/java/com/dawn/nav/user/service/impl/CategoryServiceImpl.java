...
public class CategoryServiceImpl implements CategoryService {
  ...
  @Override
  public List<CategoryVO> getCategoryTreeList(String username, String admin) {
    // 获取 userId 和 adminId
    Long userId = userMapper.findUserByUsername(username).getId();
    Long adminId = userMapper.findUserByUsername(admin).getId();

    // 查询用户的分类信息
    List<Category> userCategories = findAll(userId);
    List<Category> adminCategories = findAll(adminId);

    // 合并两个分类列表并去重
    List<Category> allCategories = new ArrayList<>(adminCategories);
